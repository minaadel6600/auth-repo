...
export default async function auth(req: IRequest, res: Response, next: NextFunction) {
    try {
        let userRepository = new UserRepository();
        const authHeader = req.headers.authorization;
        const token = authHeader && authHeader.split(' ')[1];

        if (!token) throw new HttpError(401, getTranslatedMessage(req, 'INVALID_TOKEN'));

        try {
            await verifyAccessToken(token);
            const decodedPayload = getUserPayloadFromAccessToken(token) as any; 
            req.user = await userRepository.getByID(decodedPayload._id);
            next(); 
